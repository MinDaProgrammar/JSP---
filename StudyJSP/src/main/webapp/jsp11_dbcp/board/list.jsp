...
<body>
	<section id="listForm">
		...
		<%}else{ 
 		...
 		%>
			<tr>
				<td><%=dto.getIdx() %></td>
				<td><a href="detail.jsp?idx=<%=dto.getIdx()%>"><%=dto.getSubject() %></a></td>	<!-- 이름 클릭 시 내용을 보여줄 detail페이지로 이동 -->
				<td><%=dto.getName() %></td>
				<td><%=dto.getDate() %></td>
	...
	<section id="pageList">
		<input type="button" value="이전" <%if(pageNum>1){ %>onclick="location.href='list.jsp?pageNum=<%=pageNum-1%>'"<%}else{ %>disabled="disabled"<%} %>>
...
<!-- // 		} -->
<%-- 		%> --%>
		<input type="button" value="다음" <%if(pageNum<maxPage){ %>onclick="location.href='list.jsp?pageNum=<%=pageNum+1%>'"<%} else{%>disabled="disabled"<%} %>>
	</section>
</body>
</html>